...
%       p, gof = plpva_example.plpva_example(x, 1)
% 
% Notes:
% 
...
%    
%       p, gof = plpva_example.plpva_example(x, 1, vec=numpy.arange(1.001, 5.002, 0.001))
%
...
%    
%       p, gof = plpva_example.plpva_example(x, 1, reps=10000)
% 
    """
    ...
    if (min(x) > 1000 and len(x) > 100) or sum(x - numpy.floor(x)):
        ...
        for B in range(reps):
            ...
            try:
                qmins = qmins[
                    numpy.array(numpy.unique(numpy.round(numpy.linspace(1, len(qmins), kwargs['sample']) - 1)),
                                dtype='int32')]
            except KeyError:
                pass
            dat = numpy.array([])
    ...
    # discrete method
        ...
        mmax = 20 * xmax
        pdf = numpy.r_[numpy.zeros(int(xmin - 1)), (numpy.arange(xmin, mmax + 1) ** -alpha) / zvec[I]]
        ...
        for B in range(reps):
            # semi-parametric bootstrap of data
            n1 = sum(numpy.random.random(int(N)) > pz)
            q1 = y[numpy.array(numpy.floor(ny * numpy.random.random(n1)), dtype='int32')]
            n2 = N - n1

            # simple discrete zeta generator
            r2 = numpy.sort(numpy.random.random(int(n2)))
            c = 0
            q2 = numpy.zeros(int(n2))
            k = 0
