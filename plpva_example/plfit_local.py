...
%
    ...
    try:
        xmins = xmins[
            numpy.array(numpy.unique(numpy.round(numpy.linspace(1, len(xmins), kwargs['sample']) - 1)), dtype='int32')]
    except KeyError:
        pass
    ...
    # discrete method
        ...
        if len(xmins) == 0:
            ...
            if finite:
                ...
                if n < 50:
                    print('(PLFIT) Warning: finite-size bias may be present (n = %d).' % n)
            L = - alpha * sum(numpy.log(z)) - n * numpy.log(
                scipy.special.zeta(alpha, xmin) - scipy.special.zeta(alpha, xmax + 1))
