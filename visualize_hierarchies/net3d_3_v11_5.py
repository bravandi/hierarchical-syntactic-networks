...
class netRads(netSimple):
    ...
    def set_radii(self):
        ...
        rs = lambda x: ((x ** (d - 1)).sum(-1)) ** (
                    1. / (d - 1)) + 1e-5  # use root sum squared of weights as radii for 3D
        # rs = lambda x: (4*pi*(x**(d-1)).sum(-1))+1e-5 # use root sum squared of weights as radii for 3D
    ...
    def comp_f_long(self):
        ...
        self.f_long = self.params['long_rep'] * self.params['A'] * self.dp / (self.dpn ** (d - 1) + 1e-3) ** (
                    d / (d - 1))

    def comp_thermal_noise(self):
        self.noise = self.params['links']['T0'] * exp(- self.it_num / self.params['links']['ce']) * random.randn(
            *self.pts.shape)
...
def iter_converge(self, its='auto', max_its=8e4, draw=True, verbose=True, eps=0.1, save_path=None, save_its=False,
                  **kw):
    ...
    if not hasattr(self, 'gnam0'):
        self.net_name0 = self.net_name + ''
    if its == 'auto':
    ...
    while (self.it_num < max_its) and not ck:
        if save_path:
            if not save_its:
                self.save(save_path, tv=1)
            else:
                self.net_name = self.net_name0 + '-it' + str(self.it_num)
                self.save(save_path, tv=0)
        self.params['long_rep'] = 1e-3 + 100. / (self.it_num + 1)
        ct = (eps * max([r3, self.pts.std() / self.pts.shape[0] ** (1. / 3)]) + 10. / (
                    self.it_num + 1) if simple else 'auto')
        if simple and 'ct' not in kw:
        ...
        print('\n%d) ct=%.2g, ' % (self.it_num, (ct if simple else self.ct)) \
              + (self.net_name if not simple else '') \
              + (' ' if ck else ' Has Not ') + 'Converged! dlog<l>/dlog(t)~%.3g' % (gm), "\nChecks:", ch)


################################
# Main FUEL Simulation Classes #
################################
...
class networkBase():
    def __init__(self, pts=[], edg=[], fixed=False, JSON=None, keep_paths=False,
        ...
        self.get_params(fixed=fixed, keep_paths=keep_paths,
                        PAIRS=PAIRS, POW=POW, POWn=POWn, POW_SN=POW_SN,
                        max_workers=max_workers, **kw)
        self.make_data(pts, edg, JSON=JSON, **kw)
        self.init_vars()
        self.make_network()
        self.net_name = 'net-sim-' + (
            os.path.split(JSON)[-1].strip('.json') if JSON and type(JSON) == str else 'n%d-L%d' % (
            self.nodes.points.shape.as_list()[0], self.links.points.shape.as_list()[0])) \
                        + '-th-r%.3g-segs%d-ka%.3g' % (self.links.thickness.mean() / self.nodes.radius,
                                                       self.links.segs,
                                                       self.params['links']['k'] / self.params['links']['amplitude']) \
                        + ('-fixed' if fixed else '') + ('-keep_paths' if keep_paths else '')

        self.setup_dynamics()

    def get_params(self, fixed=False, JSON=None, keep_paths=False,
                   PAIRS=PAIRS, POW=POW, POWn=POWn, POW_SN=POW_SN,  # power extra factors of r_i+r_j in node repulsion
                   max_workers=20, **kw):
        self.params = {
            'links':
    ...
    def make_data(self, pts=[], edg=[], JSON=None, **kw):
        ...
        if not JSON:
            ...
            if len(array(edg)[0]) == 2 and self.params['links']['weighted']:
                print('No edge weights given! Ignoring "weighted"...')
                self.params['links']['weighted'] = False
    ...
    def get_JSON(self, JSON, **kw):
        ...
        for k in js['info']:
            if k != 'nodes' and k != 'links':
                self.params[k] = js['info'][k]
            else:
                self.params[k].update(js['info'][k])
    ...
    def make_JSON(self):
        ...
        self.netJSON['name'] = self.net_name
...
class NetNodes():
    ...
    def get_degreesRS(nodes):
        ...
        # rs = lambda x : (4*pi*array(x)**2).sum(-1)
        nodes.degrees = array([(rs(nodes.deg_dict[i]) if i in nodes.deg_dict else 1e-2) for i in nodes.labels])
    ...
    def force_node_repel_idx(nodes, idx):
        ...
        fmat = th[:, :, newaxis] ** nodes.net.POW_SN * A * r * tf.expand_dims(
            (rlen / th) ** (nodes.net.POWn - 2) * tf.exp(-(rlen / th) ** nodes.net.POWn), 2)
    ...
    def force_end(nodes):
        ...
        nodes.Force_End = tf.concat([[tf.add(array([0] * len(nodes.net.pts[0]), dtype=float32),
                                             tf.reduce_sum([nodes.net.links.Force_Int[j] for j in nodes.link_ends[k]],
                                                           0))]
